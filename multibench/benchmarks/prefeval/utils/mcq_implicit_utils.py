...
def load_files_implicit(args):
    # system prompt:
    with open(
        f"{args.dir}/preference_dataset/implicit_preference_benchmark/type_{args.mcq}/{args.topic}.json",
        "r",
    ) as infile:
        existing_data = json.load(infile)
    # check if this directory does not exist, create one:
    dir_path = f"{args.dir}/benchmark_results/implicit/{args.task}_{args.mcq}/{args.topic}/"
    if not os.path.exists(dir_path):
        os.makedirs(dir_path)
    save_file = f"{args.dir}/benchmark_results/implicit/{args.task}_{args.mcq}/{args.topic}/{args.model}_{args.topic}_{args.inter_turns+2}turn.json"
...
def load_files_implicit_mcq(args):
    ...
    if args.pref_type == "imp_mcq":
        ...
        with open(
            f"{args.dir}/preference_dataset/implicit_preference_benchmark/persona_conversation/{args.topic}.json",
            "r",
        ) as infile:
            topic_data = json.load(infile)
    # check if this directory does not exist, create one:
    dir_path = (
        f"{args.dir}/benchmark_results/mcq_results/{args.pref_type}_{args.mcq}_{args.task}/{args.topic}/"
    )
    if not os.path.exists(dir_path):
        os.makedirs(dir_path)
    save_file = f"{args.dir}/benchmark_results/mcq_results/{args.pref_type}_{args.mcq}_{args.task}/{args.topic}/{args.model}_{args.topic}_{args.inter_turns+2}turn.json"
...
def load_files_implicit_rag(args):
    if args.pref == "mcq":
        with open(
            f"{args.dir}/preference_dataset/implicit_preference_benchmark/type_a/{args.topic}.json",
            "r",
        ) as infile:
            topic_data = json.load(infile)
        ...
        with open(
            ...
            topic_data = json.load(infile)
    dir_path = f"{args.dir}/benchmark_results/implicit/{args.task}_{args.pref}/{args.topic}/"
    if not os.path.exists(dir_path):
        os.makedirs(dir_path)
    save_file = f"{args.dir}/benchmark_results/implicit/{args.task}_{args.pref}/{args.topic}/{args.model}_{args.topic}_{args.inter_turns+2}turn.json"

    with open(
        "{args.dir}/lmsys-chat-1m/filtered_inter_turns.json",
        "r",
    ) as infile:
        turns_data = json.load(infile)

    with open(
        f"{args.dir}/preference_dataset/simcse_top15_retrived_history_implicit_{args.pref}/{args.topic}_overall300_topk_history_{args.pref}.json",
        "r",
    ) as infile:
        rag_data = json.load(infile)

    with open(
        f"{args.dir}/preference_dataset/simcse_question_inter_similarities/{args.topic}_300_inter_similarities.json",
        "r",
    ) as infile:
...
def load_files_implicit_persona(args):
    ...
    with open(
        ...
        existing_data = json.load(infile)
    # check if this directory does not exist, create one:
    dir_path = f"{args.dir}/benchmark_results/implicit/{args.task}/{args.topic}/"
    if not os.path.exists(dir_path):
        os.makedirs(dir_path)
    save_file = f"{args.dir}/benchmark_results/implicit/{args.task}/{args.topic}/{args.model}_{args.topic}_{args.inter_turns+2}turn.json"
    return existing_data, save_file
...
def load_files_implicit_selfcritic(args):
    ...
    if args.pref_type == "mcq":
        with open(
            f"{args.dir}/benchmark_results/mcq_results/imp_mcq_{args.mcq}_zero-shot/{args.topic}/{args.model}_{args.topic}_{args.inter_turns+2}turn.json",
            "r",
        ) as infile:
            existing_data = json.load(infile)
    elif args.pref_type == "persona":
        with open(
            f"{args.dir}/benchmark_results/mcq_results/imp_persona_{args.mcq}_zero-shot/{args.topic}/{args.model}_{args.topic}_{args.inter_turns+2}turn.json",
            "r",
        ) as infile:
            existing_data = json.load(infile)
    # check if this directory does not exist, create one:
...
def load_files_implicit_selfcritic_mcq(args):
    ...
    if args.pref_type == "mcq":
        ...
        with open(
            ...
            existing_data = json.load(infile)
    # check if this directory does not exist, create one:
    dir_path = f"{args.dir}/benchmark_results/mcq_results/imp_{args.pref_type}_{args.task}/{args.topic}/"
    if not os.path.exists(dir_path):
        os.makedirs(dir_path)
    save_file = f"{args.dir}/benchmark_results/mcq_results/imp_{args.pref_type}_{args.task}/{args.topic}/{args.model}_{args.topic}_{args.inter_turns+2}turn.json"
    return existing_data, save_file
